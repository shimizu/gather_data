...
5. ユーザーに結果をわかりやすく報告する（データソースやデータセットに言及する際は必ずURLを併記すること）
6. 調査結果を報告する際は save_report でレポートファイルも出力する
...
const tools: Anthropic.Tool[] = [
  ...
  },
  {
    name: "save_report",
    description:
      "調査結果をMarkdownレポートとしてファイルに保存します。ユーザーに結果を報告する際にコンソール出力とは別にレポートファイルを出力してください。",
    input_schema: {
      type: "object" as const,
      properties: {
        title: {
          type: "string",
          description: "レポートのタイトル（ファイル名にも使用）",
        },
        content: {
          type: "string",
          description: "Markdown形式のレポート本文",
        },
      },
...
async function executeTool(
  ...
  switch (name) {
    ...
    case "save_report":
      return saveReportTool(input.title as string, input.content as string);
    default:
      return `未知のツール: ${name}`;
  }
}
